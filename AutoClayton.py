...
class PictureMoverApp:
    def __init__(self, root):
        ...
        self.style.configure("TButton", background="#4CAF50", foreground="white", font=("Helvetica", 10, "bold"), padx=10, pady=5)
        self.style.configure("TLabel", background="#F0F0F0", font=("Helvetica", 10), pady=5)
        ...
        self.left_choose_button = ttk.Button(self.left_frame, text="Choose Directory", command=self.choose_directory)
        ...
        self.right_choose_button = ttk.Button(self.right_frame, text="Choose Picture Folder", command=self.choose_picture_folder)
        self.right_choose_button.pack(pady=10)

        self.move_button = ttk.Button(self.root, text="Move Pictures", command=self.move_pictures)
    ...
    def choose_directory(self):
        ...
        if directory_path:
            self.subfolder_paths = sorted([os.path.join(directory_path, d) for d in os.listdir(directory_path) if os.path.isdir(os.path.join(directory_path, d))])
            self.left_listbox.delete(0, tk.END)
            for path in self.subfolder_paths:
                self.left_listbox.insert(tk.END, os.path.basename(path))
            self.left_total_label.config(text="Total Subfolders: {}".format(len(self.subfolder_paths)))

    def choose_picture_folder(self):
        directory_path = filedialog.askdirectory()
        if directory_path:
            self.picture_folder_path = directory_path
            picture_files = sorted([f for f in os.listdir(directory_path) if os.path.isfile(os.path.join(directory_path, f))])
            self.right_listbox.delete(0, tk.END)
            for file in picture_files:
                self.right_listbox.insert(tk.END, file)
            self.right_total_label.config(text="Total Pictures: {}".format(len(picture_files)))
            ...
            if len(self.subfolder_paths) > 0:
                ...
                self.ratio.config(text="Ratio: {:.2f} pictures per subfolder".format(ratio))
            else:
    ...
    def move_pictures(self):
        if not self.picture_folder_path or not self.subfolder_paths:
            messagebox.showerror("Error", "Please choose both a picture folder and a directory with subfolders.")
            return
        ...
        if num_pictures % num_subfolders != 0:
            messagebox.showerror("Error", "The ratio of pictures to subfolders is not a whole number. Please adjust your selection.")
            return
        ...
        for i, subfolder_path in enumerate(self.subfolder_paths):
            ...
            for j in range(num_to_move):
                ...
                shutil.copy(os.path.join(self.picture_folder_path, picture_name), os.path.join(destination_folder, picture_name))
                print("Moved {} to {}".format(picture_name, os.path.basename(subfolder_path)))
